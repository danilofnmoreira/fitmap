...
import com.fitmap.function.setroles.payload.request.SetRolesRequest.UserType;
...
class SetRolesFunctionTest {
    ...
    void should_return_emtpy_response_when_success() throws Exception {
        ...
        when(request.getInputStream()).thenReturn(new ByteArrayInputStream(createJson(new SetRolesRequest("idToken", UserType.GYM)).getBytes()));
    ...
    class WhenErrors {
        ...
        void should_answer_Bad_Request_if_request_body_is_not_readable() throws Exception {
            ...
            assertThat(actual)
                .extracting("status", "statusError", "message", "path")
                .containsExactly(400, "Bad Request", "Cannot convert request body in SetRolesRequest; nested exception is com.fasterxml.jackson.databind.exc.InvalidFormatException: Cannot deserialize value of type `com.fitmap.function.setroles.payload.request.SetRolesRequest$UserType` from String \"fake_user_type\": not one of the values accepted for Enum class: [STUDENT, PERSONAL_TRAINER, GYM]\n at [Source: (ByteArrayInputStream); line: 1, column: 15] (through reference chain: com.fitmap.function.setroles.payload.request.SetRolesRequest[\"user_type\"])", "/test");
        ...
        void should_answer_Bad_Request_if_there_are_request_constraints_violations() throws Exception {
            ...
            when(request.getMethod()).thenReturn("POST");
            when(request.getContentType()).thenReturn(Optional.of(MediaType.APPLICATION_JSON_VALUE));
            when(request.getInputStream()).thenReturn(new ByteArrayInputStream(createJson(new SetRolesRequest()).getBytes()));
        ...
        void should_answer_InternalServerError_if_some_unexpected_exception_occur() throws Exception {
            ...
            when(request.getContentType()).thenReturn(Optional.of(MediaType.APPLICATION_JSON_VALUE));
            when(request.getInputStream()).thenReturn(new ByteArrayInputStream(createJson(new SetRolesRequest("idToken", UserType.GYM)).getBytes()));
            doThrow(new RuntimeException("message")).when(setRolesService).setRoles(any());
        ...
        void should_answer_an_terminal_known_error_if_some_expected_exception_occur() throws Exception {
            ...
            when(request.getContentType()).thenReturn(Optional.of(MediaType.APPLICATION_JSON_VALUE));
            when(request.getInputStream()).thenReturn(new ByteArrayInputStream(createJson(new SetRolesRequest("idToken", UserType.GYM)).getBytes()));
            doThrow(new TerminalException("message", HttpStatus.BAD_REQUEST)).when(setRolesService).setRoles(any());
